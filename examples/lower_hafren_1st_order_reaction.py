...
# =====================================
# Load the data
data = pd.read_csv('lower_hafren_data_1st_order_reaction.csv', index_col=0, parse_dates=True)
...
# =====================================================================
#%% Convolve the transit time distributions with the input concentrations for Cl
# =====================================================================
# Use the estimated transit time distribution and input timeseries to estimate
# the output timeseries
C_out, C_mod_raw, observed_fraction = rsas.transport_with_evapoconcentration_1st_order_reaction(PQ, 
                                      thetaQ, thetaS, data['P+occult Cl mg/l'], C_old,
                                      k1=0.0, C_eq=0.0, C_sur=0.0)
data['Q Predicted Cl mg/l'] = C_out
# Plot the result
fig = plt.figure(1)
plt.clf()
plt.plot(data.index, data['Q Cl mg/l'], label='Observed Cl')
plt.plot(data.index, data['Q Predicted Cl mg/l'], label='Predicted Cl')
plt.title('Cl Prediction / Observation (k1=0)')
plt.legend(loc=1, ncol=2)

# =====================================================================
#%% Convolve the transit time distributions with the input concentrations for Mg
# =====================================================================
# Use the estimated transit time distribution and input timeseries to estimate
# the output timeseries
C_out, C_mod_raw, observed_fraction = rsas.transport_with_evapoconcentration_1st_order_reaction(PQ, 
                                      thetaQ, thetaS, data['P Mg mg/l'], C_old=.8,
                                      k1=0.01, C_eq=0.8, C_sur=0.0)
data['Q Predicted Mg mg/l'] = C_out
# Plot the result
fig = plt.figure(2)
plt.clf()
plt.plot(data.index, data['Q Mg mg/l'], label='Observed Mg')
plt.plot(data.index, data['Q Predicted Mg mg/l'], label='Predicted Mg')
plt.legend(ncol=2)
plt.title('Mg Prediction / Observation (k1=0.8, C_eq=0.0)')

np.corrcoef(data['Q Mg mg/l'], C_out)