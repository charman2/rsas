...
fig = plt.figure(2)
plt.clf()
plt.step(np.arange(N), C_Q1em, 'r', linestyle='-', label='mean exact', lw=2, where='post')
plt.step(np.arange(N), C_Q1m1, 'g', linestyle='--', label='mean rsas internal', lw=2, where='post')
plt.step(np.arange(N), C_Q1m2, 'b', linestyle=':', label='mean rsas.transport', lw=2, where='post')
